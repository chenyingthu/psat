...
Psat::~Psat(){
	...
	delete []solver.jfng.gmres.h;
	delete []solver.jfng.gmres.z;
	//delete []solver.jfng.gmres.v;
	delete []solver.jfng.gmres.c;
	delete []solver.jfng.gmres.s;
//	delete []solver.P;
	delete []solver.deltx;
	delete []solver.delty;
	delete []step;
...
void Psat::specifySystem(){
	double type=0;
	int i=0;
	FILE *fp=fopen("D:\\home\\yzt\\bishe\\psat\\psat_c\\caseDfn\\d_039ieee.dat","r");
...
void Psat::fm_int_step(double t,double h,double *tempi,int k){
	...
	while(err_max>settings.dyntol&&iterazione<=settings.dynmit){
		...
		case 2:
			for ( int i = 0; i < dae.n; i += 1 ) {
				dae.tn[i]=dae.x[i]-xa[i]-h*0.5*(dae.f[i]+fn[i]);
			}
			break;
		default:
			break;
...
*  JFNG
*-----------------------------------------------------------------------------*/
void Psat::dyn_f_iniSolver(int iFlag){
	if ( iFlag==1 ) {
		solver.jfng.newton.tol=1e-4;
		solver.jfng.newton.maxit=30;
		solver.jfng.newton.debug=1;
		solver.jfng.newton.method=1;
		solver.jfng.gmres.tol_fixed=0.1;
		solver.jfng.gmres.tol=0.1;
		solver.jfng.gmres.maxit=50;
		solver.jfng.gmres.reorth=2;
		solver.jfng.precond.inner=1;
		solver.jfng.precond.outer=1;
		solver.jfng.precond.innerStop=2;
		solver.jfng.precond.outerStop=2;
		solver.jfng.findiff=1e-7;
		int kmax=solver.jfng.gmres.maxit;
		int n=dae.n+2*bus.n;
		solver.jfng.gmres.h=new double [kmax*kmax];
		solver.jfng.gmres.z=new double [n*kmax];
		solver.jfng.gmres.v=new double [n*kmax];
		solver.jfng.gmres.c=new double [kmax+1];
		solver.jfng.gmres.s=new double [kmax+1];
		memset(solver.jfng.gmres.h,0,kmax*kmax*sizeof(double));
		memset(solver.jfng.gmres.z,0,n*kmax*sizeof(double));
		step=new double [n];
		...
		cudaMalloc((void**)&solver.deltx_dev,n*solver.update.maxnum * sizeof(double));
		cudaMalloc((void**)&solver.delty_dev,n*solver.update.maxnum * sizeof(double));
		solver.update.dest=10;
	}
	else if(iFlag==2){
...
void Psat::dyn_f_iniNet(int iFlag){
	specifySystem();
	init();
	formConMatrix();
	initialLF();
}
void Psat::dyn_f_iniSyn(int iFlag){
	fm_spf();
}
void Psat::dyn_f_iniDae(int iFlag){
	fm_int_intial();
	dae.X=new double [dae.n+2*bus.n];
	V_bak=new double [bus.n];
	formDAEX();
}
void Psat::formDAEX(){
	int k=0;
...
void Psat::dyn_f_iniSimu(int iFlag){
	...
	simu.t_next=simu.tStep;
	simu.converged=1;
	simu.newton_iteration=0;
	simu.neval=0;
...
void Psat::updatePreconditioner(int iFlag){
	...
	while(solver.update.last_num+1<solver.update.num){
		for ( int i = 0; i < n; i += 1 ) {
			p[i]=solver.update.dest*solver.deltx[i+solver.update.last_num*n];
			q[i]=solver.delty[i+solver.update.last_num*n];
		}
		temp=0;
		for ( int j = 0; j < solver.update.last_num; j += 1 ) {
			temp=0;
			for ( int i = 0; i < n; i += 1 ) {
				temp+=solver.deltx[i+j*n]*q[i];
			}
			for ( int i = 0; i < n; i += 1 ) {
				q[i]+=temp*solver.delty[i+j*n];
			}
		}
		temp=0;
		for ( int i = 0; i < n; i += 1 ) {
		...
		// printf("test\t%lf\n",temp);
		for ( int i = 0; i < n; i += 1 ) {
			solver.deltx[i+solver.update.last_num*n]=p[i]/temp;
			solver.delty[i+solver.update.last_num*n]=p[i]-q[i];
			// printf("%lf\t%lf\t%lf\t%lf\n",p[i],temp,solver.deltx[solver.update.last_num+i*solver.update.maxnum],solver.delty[solver.update.last_num+i*solver.update.maxnum]);
		}
...
void Psat::pre_gmres(double *f0,double *xc,double *x){
	...
	for ( int i = 0; i < kmax+1; i += 1 ) {
		solver.jfng.gmres.s[i]=0;
		solver.jfng.gmres.c[i]=0;
		g[i]=0;
	}
	for (int i = 0;i < n;++i){
		x[i]=0;
	...
	while((rho > errtol)&&(k<kmax)){
		...
		int kk=k-1;
		for ( int i = 0; i < n; i += 1 ) {
			solver.jfng.gmres.z[i+kk*n]=solver.jfng.gmres.v[i+kk*n];
		}
		if(solver.jfng.precond.inner==1||solver.jfng.precond.outer==1){
	...
	// memset(h_temp,0,k*k*sizeof(double));
	int *ipiv=new int[k];
	for ( int i = 0; i < k; i += 1 ) {
		for ( int j = 0; j < k; j += 1 ) {
...
void Psat::precondition(double *x,int k){//todo multiSteps
	...
	int n=dae.n+2*bus.n;
	for(int i = 0;i < n;++i)
		x[i+k*n]=x[i+k*n]*solver.update.dest;
	for ( int i = 0; i <solver.update.last_num; i += 1 ) {
		double temp=0;
		for ( int j = 0; j < n; j += 1 ) {
			temp+=solver.deltx[j+i*n]*x[j+k*n];
		}
		for ( int j = 0; j < n; j += 1 ) {
			x[j+k*n]=x[j+k*n]+temp*solver.delty[j+i*n];
		}
	}
}
void Psat::dirder(double *x,double *w,double *f0,double *z){
	int n=dae.n+2*bus.n;
